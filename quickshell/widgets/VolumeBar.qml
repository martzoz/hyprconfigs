...
ShellRoot {
    ...
    // ── Event-driven volume via pactl subscribe ──
    Process {
        id: volumeSubscribe
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (lines.length >= 1) {
                    ...
                    if (!isNaN(v)) {
                        var newVol = Math.max(0, Math.min(1, v / 100))
                        if (newVol !== root.volume) {
                            root.volume = newVol
                            // Show the bar when volume changes externally
    ...
    // ── Timer to show bar on external volume change ──
    Timer {
        id: externalChangeTimer
        interval: 3500
        repeat: false
        onTriggered: root.userInteracting = false
        onRunningChanged: if (running) root.userInteracting = true
    }
    ...
    Variants {
        ...
        PanelWindow {
            ...
            property bool revealed: root.userInteracting
