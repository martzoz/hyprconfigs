...
Item {
    ...
    // Known terminal-only apps that need kitty to run
    readonly property var terminalApps: ["yazi","btop","htop","nvim","vim","helix","hx","fish","bash","zsh","ranger","nnn","mc","cmus","ncmpcpp","pulsemixer","nmtui","lazygit","lazydocker"]

    function isTerminalApp(cmd) {
        var base = cmd.split(" ")[0].split("/").pop().toLowerCase()
        for (var i = 0; i < terminalApps.length; i++) {
            if (base === terminalApps[i]) return true
        }
        return false
    }
    ...
    function launchApp(cmd) {
        ...
        if (isTerminalApp(cmd)) {
            finalCmd = "nohup kitty fish -c '" + cmd + "' > /dev/null 2>&1 &"
        } else {
            finalCmd = "nohup " + cmd + " > /dev/null 2>&1 &"
        }
        launchProc.pending = finalCmd
        ...
        Rectangle {
            ...
            Item {
                ...
                Row {
                    ...
                    Item { width:parent.width - 510; height:1 }
                    Row {
                        spacing:14; anchors.verticalCenter:parent.verticalCenter
                        Item {
                        ...
                        ListView {
                            ...
                            delegate: Item {
                                ...
                                Row {
                                    ...
                                    Item { width:appList.width-465; height:1 }
                                    Text {
                                        anchors.verticalCenter:parent.verticalCenter
            ...
            Item {
                ...
                Row {
                    ...
                    Item { width:parent.width-570; height:1 }
                    Row {
                        spacing:14; anchors.verticalCenter:parent.verticalCenter
                        Repeater {
